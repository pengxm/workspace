$(function(){
    ...
    */
    ...
    var telValidate = new Validate('.tel', '.tel-txt', /^0?1[3|4|5|8][0-9]\d{8}$/, '格式正确', '格式错误');
    ...
    validateInput.addEventListener("keyup", function(event){
        ...
        if($('.validate-input').val() === $('.validate-val').text() && $('.validate-input').val() !== ''){
            $('.validate-txt').html('验证码正确');
        }else {
            $('.validate-txt').html('输入错误');
        }
    });
    ...
    // 密码验证
    var pwdVal = new Validate('.pwd-value', '.pwd-txt',/^(\w){5,17}$/, '格式正确', '格式错误');
    pwdVal.validate();

$('.submit-btn').click(function(){
    if($('.tel-txt').text()==='格式正确' && $('.validate-txt').text() === '验证码正确' && $(".pwd-txt").text()==="格式正确"){
        console.log('right');
    } else {
