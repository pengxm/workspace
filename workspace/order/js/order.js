$(function(){
    // 适应屏幕大小
    document.querySelector("html").style.fontSize=(function(){return document.documentElement.clientWidth/375*62.5+"%";
    }());
    ...
    $('.submit-btn').click(function(){
        if($('.order-center input').eq(0).is(":checked")){
            alert(1)
        }else if($('.order-center input').eq(1).is(":checked")){
            alert(2)
        }
    });
    ...
    */
    ...
    var telValidate = new Validate('.tel', '.tel-txt', /^0?1[3|4|5|8][0-9]\d{8}$/, '正确', '错误');
    ...
    validateInput.addEventListener("keyup", function(event){
        ...
        if($('.validate-input').val() === $('.validate-val').text()){
            $('.validate-txt').html('right');
        }else {
            $('.validate-txt').html('error');
        }
    });
    ...
    // 密码验证
    var pwdVal = new Validate('.pwd-value', '.pwd-txt',/^(\w){5,17}$/, '正确', '错误');
    pwdVal.validate();
